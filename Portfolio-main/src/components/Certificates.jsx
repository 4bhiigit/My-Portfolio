...
const CertificateCard = ({ cert, aosDelay, isNew }) => (
  <a 
    href={cert.credentialUrl}
    target="_blank"
    rel="noopener noreferrer"
    data-aos={isNew ? "" : "zoom-in"}
    data-aos-delay={isNew ? "" : aosDelay}
    className={`bg-black/20 backdrop-blur-sm rounded-2xl p-5 border border-white/10 hover:border-white/25 hover:scale-[1.03] hover:bg-black/30 hover:shadow-[0_15px_40px_rgba(0,0,0,0.3)] transition-all duration-500 cursor-pointer block group ${
  ...
  >
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-red-200 uppercase tracking-widest group-hover:underline">
          Verify Certificate
    ...
    <section className="bg-[#ff2a2a] pt-20 pb-28 px-6 md:px-12 w-full relative overflow-hidden font-sans">
      ...
      <div className="max-w-6xl mx-auto relative z-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-12">
          {(isExpanded ? certificates.all : certificates.featured).map((cert, index) => {
            const isNewlyAdded = isExpanded && index >= certificates.featured.length;
            return (
              <CertificateCard 
                key={cert.name} 
                cert={cert} 
        ...
        <div data-aos="fade-up" data-aos-delay="700" className="flex justify-center">
          <button
            ...
            className="flex items-center gap-3 px-8 py-3.5 rounded-full bg-white text-black font-bold text-base hover:bg-gray-100 hover:scale-105 hover:shadow-[0_10px_30px_rgba(0,0,0,0.2)] transition-all duration-300 group cursor-pointer"
          >
            <svg className="w-5 h-5 text-[#ff2a2a] transform group-hover:rotate-12 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            {isExpanded ? "Show Less" : "View All Certificates"}
            <svg className={`w-4 h-4 transform transition-transform ${isExpanded ? 'rotate-180' : 'group-hover:translate-x-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
